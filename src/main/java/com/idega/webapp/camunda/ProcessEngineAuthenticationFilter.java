 ...
 */
package com.idega.webapp.camunda;
 ...
 * <thead> <tbody>
 * <tr>
 * <td>{@value #AUTHENTICATION_PROVIDER_PARAM}</td>
 * <td>yes</td>
 * <td>An implementation of {@link AuthenticationProvider}</td>
 * </tr>
 * <tr>
 * <td>{@value #SERVLET_PATH_PREFIX}</td>
 * <td>no</td>
 * <td>The expected servlet path. Should only be set, if the underlying JAX-RS
 * application is not deployed as a servlet (e.g. Resteasy allows deployments as
 * a servlet filter). Value has to match what would be the
 * {@link HttpServletRequest#getServletPath()} if it was deployed as a
 * servlet.</td>
 * </tr>
 * </tbody>
 * </table>
...
public class ProcessEngineAuthenticationFilter implements Filter {
	...
	protected static final Pattern ENGINE_REQUEST_URL_PATTERN = Pattern
			.compile("^" + NamedProcessEngineRestServiceImpl.PATH + "/(.*?)(/|$)");
	...
	protected void setAuthenticatedUser(ProcessEngine engine, String userId, List<String> groupIds,
	...
	protected String extractEngineName(String requestUrl) {
		...
		if (matcher.find()) {
			...
			// any request that does not match a specific engine and is not an /engine
			// request
